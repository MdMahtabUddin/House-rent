        ...
        <DropdownMenu>
          ...
          {userAccess?.house && userAccess?.shop && (
            <DropdownMenuContent align="start" className="w-[200px]">
              <DropdownMenuItem onClick={() => { setPropertyType('House'); onNavigate?.(); }} className="cursor-pointer">
                <Home className="mr-2 h-4 w-4 text-green-600" />
                <span>House Rent</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => { setPropertyType('Shop'); onNavigate?.(); }} className="cursor-pointer">
